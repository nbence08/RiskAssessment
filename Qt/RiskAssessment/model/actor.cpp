...
Actor::Actor(QObject* parent):Model("AR", parent){
}

Actor::Actor(QString name, QObject* parent): Model(name, "AR", parent) {
}

std::unique_ptr<ModelVisitor> Actor::getGrantPAccessVisitor() {
    return std::make_unique<ActorGrantPAccessVisitor>(*this);
}

std::unique_ptr<ModelVisitor> Actor::getGrantLPrivVisitor() {
    return std::make_unique<ActorGrantLPrivVisitor>(*this);
}

std::unique_ptr<ModelVisitor> Actor::getGrantConnectionVisitor() {
    return std::make_unique<ActorGrantConnectionVisitor>(*this);
}

void Actor::addLPriv(std::weak_ptr<LPriv> lpriv){
    lPrivs.push_back(lpriv);
}

void Actor::removeLPrivForOperation(QString opName){
...
void Actor::removeLPrivLike(QString name){
...
void Actor::removeConnectionTo(QString serviceName) {
...
void Actor::removePAccessTo(QString paccName) {
...
void Actor::loadConnections(QJsonObject jsonObj, std::unordered_map<QString, Serializable*> addressTable){
...
Actor::ActorGrantPAccessVisitor::ActorGrantPAccessVisitor(Model& model): ModelVisitor(&model) {}

void Actor::ActorGrantPAccessVisitor::visits(PhysicalZone& pzone) {
    auto paccess = pzone.getPAccess();
    ((Actor*)this->model)->pAccesses.push_back(paccess);
...
void Actor::ActorGrantPAccessVisitor::visits(Device& /*device*/) {
    throw std::logic_error("further deliberation required");
}

Actor::ActorGrantLPrivVisitor::ActorGrantLPrivVisitor(Model& model): ModelVisitor(&model) {}

void Actor::ActorGrantLPrivVisitor::visits(Role& role){
    auto actor = static_cast<Actor*>(model);
    actor->roles.push_back(&role);

    emit actor->lPrivGranted(actor, &role);
}

void Actor::ActorGrantLPrivVisitor::visits(Operation& operation)
{
    auto lPriv = operation.getLPriv();
    ((Actor*)this->model)->lPrivs.push_back(lPriv);
    emit model->lPrivGranted(model, &operation);
}

void Actor::ActorGrantLPrivVisitor::visits(Service& /*service*/)
{
    throw std::logic_error("further deliberation required");
}

Actor::ActorGrantConnectionVisitor::ActorGrantConnectionVisitor(Model& model): ModelVisitor(&model) {}

void Actor::ActorGrantConnectionVisitor::visits(Service& service)
{
    auto conn = service.getConnection();
    ((Actor*)this->model)->connections.push_back(conn);
    emit model->connectionAddedFromTo(model, &service);
}

void Actor::ActorGrantConnectionVisitor::visits(Operation& /*operation*/)
{
    throw std::logic_error("further deliberation required");
}
